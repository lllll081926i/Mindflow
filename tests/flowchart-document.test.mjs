...
const flowchartPackageDir = path.resolve('src/services/flowchart')

const readFlowchartPackageSource = () => {
  const files = [
    flowchartDocumentPath,
    ...fs
      .readdirSync(flowchartPackageDir)
      .filter(name => name.endsWith('.js') && !name.includes('backup'))
      .map(name => path.join(flowchartPackageDir, name))
  ]
  return files.map(filePath => fs.readFileSync(filePath, 'utf8')).join('\n')
}

const bundleFlowchartDocumentSource = () => {
  const seen = new Set()
  const stripAndCollect = filePath => {
    ...
    source = source.replace(
      ...
      (full, spec) => {
        if (spec === '@/utils/json') {
          return `const parseExternalJsonSafely = value => {
  try {
...
`
        ...
        if (spec.startsWith('.')) {
          deps.push(path.normalize(path.join(path.dirname(abs), spec)))
          return '\n'
        }
        throw new Error(`Unsupported import ${spec} in ${abs}`)
      }
    )
    source = source.replace(/export\s*\*\s*from\s*['"]([^'"]+)['"]\s*;?/g, (full, spec) => {
      deps.push(path.normalize(path.join(path.dirname(abs), spec)))
      return '\n'
    })
    source = source.replace(
  ...
  const bundle = filePath => {
    const { code, deps } = stripAndCollect(filePath)
    let output = ''
  ...
  const body = bundle(flowchartDocumentPath)
  const names = [
    ...body.matchAll(/^(?:const|let|var|function|async function)\s+([A-Za-z0-9_]+)/gm)
  ]
    .map(match => match[1])
    .filter(name => name !== 'parseExternalJsonSafely')
  const uniqueNames = [...new Set(names)]
  return `${body}\nexport {\n${uniqueNames.join(',\n')}\n}\n`
}

const loadFlowchartDocumentModule = async () => {
  const source = bundleFlowchartDocumentSource()
  return import(
...
test('流程图文档服务存在并暴露核心能力', () => {
  ...
  const source = readFlowchartPackageSource()
...
test('连线路由不变量：正交路径端点方向合法且不穿节点内部', async () => {
  const { getFlowchartEdgeLayout } = await loadFlowchartDocumentModule()
  const sourceNode = { id: 'a', x: 100, y: 100, width: 160, height: 72 }
  const targetNode = { id: 'b', x: 420, y: 280, width: 160, height: 72 }
  const layout = getFlowchartEdgeLayout(
    {
      id: 'edge-invariant-1',
      source: 'a',
  ...
  assert.ok(Array.isArray(layout.pathPoints) && layout.pathPoints.length >= 2)
  assert.equal(
    getFlowchartTestPolylineInteriorHits(layout.pathPoints, [sourceNode, targetNode], [
      sourceNode.id,
      targetNode.id
    ]).length,
    0
  ...
  // Orthogonal segments should be axis-aligned.
  for (let index = 0; index < layout.pathPoints.length - 1; index += 1) {
    const start = layout.pathPoints[index]
    const end = layout.pathPoints[index + 1]
    const axisAligned =
      Math.abs(Number(start.x) - Number(end.x)) <= 0.001 ||
      Math.abs(Number(start.y) - Number(end.y)) <= 0.001
    assert.equal(axisAligned, true)
  }
})

test('连线路由不变量：近垂直布局默认使用竖向端口而非侧边出线', async () => {
  const { getFlowchartEdgeLayout } = await loadFlowchartDocumentModule()
  const layout = getFlowchartEdgeLayout(
    {
      id: 'edge-invariant-vertical',
      source: 'a',
      target: 'b',
      style: { pathType: 'orthogonal' }
    },
    { id: 'a', x: 200, y: 80, width: 160, height: 72 },
    { id: 'b', x: 230, y: 320, width: 160, height: 72 }
  )

  assert.equal(layout.sourceDirection, 'bottom')
  assert.equal(layout.targetDirection, 'top')
  assert.ok(layout.pathPoints.length <= 4)
})

test('flowchart 文档服务已拆为 facade + 子模块', () => {
  const packageSource = readFlowchartPackageSource()
  assert.match(packageSource, /export \* from '\.\/flowchart\/core\.js'/)
  assert.equal(fs.existsSync(path.resolve('src/services/flowchart/constants.js')), true)
  assert.equal(fs.existsSync(path.resolve('src/services/flowchart/themes.js')), true)
  assert.equal(fs.existsSync(path.resolve('src/services/flowchart/svgUtils.js')), true)
  assert.equal(fs.existsSync(path.resolve('src/services/flowchart/core.js')), true)
  assert.equal(fs.existsSync(path.resolve('src/services/flowchart/parts/10-templates.js')), true)
  assert.equal(fs.existsSync(path.resolve('src/services/flowchart/parts/30-routing.js')), true)
