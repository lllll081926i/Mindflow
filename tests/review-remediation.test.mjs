...
test('外部 JSON 在导入、打开文件与剪贴板路径上统一走本地解析入口', () => {
  ...
  assert.match(editorLocalFileSessionSource, /parseStoredDocumentContent\(str\)/)
...
test('工具栏本地文件读写会绑定稳定请求上下文，避免切换文件后的串写与旧请求回写', () => {
  assert.match(editorLocalFileSessionSource, /export const snapshotLocalFileRef =/)
  assert.match(editorLocalFileSessionSource, /export const isSameLocalFileRef =/)
  assert.match(editorLocalFileSessionSource, /export const parseToolbarLocalFileContent =/)
...
test('AI 供应商配置支持 OpenAI Chat Completions 与 Anthropic Messages 协议', () => {
  const openaiConfig = buildAiRequestConfig({
    provider: 'openai',
    key: 'sk-openai',
    model: 'gpt-4.1-mini'
  })
  assert.equal(openaiConfig.protocol, 'openai-compatible')
  assert.equal(openaiConfig.api, 'https://api.openai.com/v1/chat/completions')
  assert.equal(openaiConfig.headers.Authorization, 'Bearer sk-openai')
  assert.equal(openaiConfig.data.model, 'gpt-4.1-mini')
  assert.equal(openaiConfig.data.stream, true)

  const anthropicConfig = buildAiRequestConfig({
    provider: 'anthropic',
    key: 'sk-ant',
    model: 'claude-sonnet-4-5'
  })
  assert.equal(anthropicConfig.protocol, 'anthropic')
  assert.equal(anthropicConfig.api, 'https://api.anthropic.com/v1/messages')
  assert.equal(anthropicConfig.headers['x-api-key'], 'sk-ant')
  assert.equal(anthropicConfig.headers['anthropic-version'], '2023-06-01')
  assert.equal(anthropicConfig.data.model, 'claude-sonnet-4-5')
  assert.equal(anthropicConfig.data.max_tokens, 4096)
})

test('Anthropic 请求会把 system 消息提升到顶层并解析 Messages SSE 文本增量', () => {
  const data = buildAiProtocolRequestData('anthropic', {
    model: 'claude-sonnet-4-5',
    stream: true,
    messages: [
      { role: 'system', content: '只返回 JSON' },
      { role: 'user', content: '生成流程图' }
    ]
  })
  assert.equal(data.system, '只返回 JSON')
  assert.deepEqual(data.messages, [{ role: 'user', content: '生成流程图' }])

  const stream = consumeAnthropicStreamText(
    '',
  ...
  assert.equal(stream.items[0].delta.text, 'hello')
