...
test('自动修复计划提供可预览的节点/连线 diff', () => {
  ...
  assert.ok(plan.diff)
  assert.ok(Array.isArray(plan.diff.addedNodes))
  assert.ok(plan.diff.addedNodes.some(n => n.type === 'start' || n.type === 'end'))
  assert.ok(Array.isArray(plan.diff.removedEdges))
  assert.ok(plan.diff.removedEdges.some(e => e.id === 'bad'))
  assert.equal(typeof plan.diff.scoreDelta, 'number')
})
