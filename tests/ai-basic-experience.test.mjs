...
test('AI 整图/续写会在生成前快照，停止或失败时回滚', () => {
  assert.match(aiCreateSource, /captureMindMapSnapshot\(/)
  assert.match(aiCreateSource, /restoreMindMapSnapshot\(/)
  assert.match(aiCreateSource, /this\.captureMindMapSnapshot\(\)/)
  assert.match(aiCreateSource, /stopCreate\(/)
  assert.match(
    aiCreateSource,
    /activeAiRequestKind === 'create-all'[\s\S]*restoreMindMapSnapshot/
  )
  assert.match(
    aiCreateSource,
    /error => \{[\s\S]*restoreMindMapSnapshot\(\{ notify: true \}\)/
  )
